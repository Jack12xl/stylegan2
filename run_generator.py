...
def _label2str(label, spliter='-'):
    if label is None:
        return ''
    else:
        # x in range [-pi/2, pi/2]
        return spliter.join([str(int(x*180.0/math.pi)+90) for x in list(np.squeeze(label))])

# ----------------------------------------------------------------------------

def _draw(img, corner, imgpts):
...
def style_mixing_example(network_pkl, row_seeds, col_seeds, truncation_psi, col_styles, minibatch_size=4):
    ...
    all_z = np.stack([np.random.RandomState(seed).randn(*Gs.input_shape[1:])
                      for seed in all_seeds])  # [minibatch, component]
...
def _parse_label(s):
    ...
    vals = s.split(',')
    assert len(vals) > 0
    return np.asarray([float(x) for x in vals])
